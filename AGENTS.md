...
5. **Zero external dependencies where possible** — zmosh has two pinned external deps: `ghostty-vt` (terminal emulation) and `quicz` (QUIC transport, content-hash-pinned; see `docs/decisions/0001-zig-quicz-network-stack.md`). Keep them minimal. Prefer `std.crypto.aead.xchacha20poly1305` over pulling in a C crypto library.
...
Network-layer contracts and decisions live with the code:
`docs/quic-wire.md` (ZMQ1 wire protocol, normative DETACH/terminal-FIN
sequencing), `docs/quic-client-runtime.md` (client FSMs, ownership
rules, terminal-event precedence), and
`docs/decisions/0001-zig-quicz-network-stack.md` (why Zig plus pinned
quicz).
